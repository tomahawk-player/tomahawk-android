...
public class ScriptResolver implements Resolver {
    ...
    /**
     * Initialize this ScriptResolver. Loads the .js script from the given path and sets
    ...
    private void init() {
        ...
        mScriptEngine.loadDataWithBaseURL(baseurl, "<!DOCTYPE html>" + "<html>" + "<body>"
                ...
                + "<script src=\"file:///android_asset/" + mScriptFilePath + "\" type=\"text/javascript\"></script>"
                + "</body>" + "</html>", "text/html", null, null);
    }

    /**
     * This method is being called, when the ScriptEngine has completely loaded the given .js script.
     */
    ...
    private void resolverInit() {
        mScriptEngine.loadUrl("javascript:" + RESOLVER_LEGACY_CODE
                + makeJSFunctionCallbackJava(R.id.scriptresolver_resolver_init, "resolver.init()", false));
    }
    ...
    private void resolverUserConfig() {
        mScriptEngine.loadUrl("javascript:" + RESOLVER_LEGACY_CODE
                + makeJSFunctionCallbackJava(R.id.scriptresolver_resolver_userconfig, "resolver.getUserConfig()", true));
    }

    /**
     * Every callback from a function inside the javascript should first call the method callbackToJava, which is
     * exposed to javascript within the ScriptInterface. And after that this callback will be handled here.
    ...
    public void handleCallbackToJava(int id, JSONObject obj) {
        ...
        if (obj != null)
            s = obj.toString();
        Log.d(TAG, "handleCallbackToJava: id='" + mTomahawkApp.getResources().getResourceEntryName(id) + "(" + id + ")"
                + "', result='" + s + "'");
        try {
            if (id == R.id.scriptresolver_resolver_settings) {
                ...
                mIcon = Drawable.createFromStream(mTomahawkApp.getAssets().open(basepath + obj.getString("icon")), null);
    ...
    /**
     * Wraps the given js call into the necessary functions to make sure, that the javascript function will callback
    ...
    /**
     * @return the Drawable which has been created by loading the image the js function attribute "icon" pointed at
     */
